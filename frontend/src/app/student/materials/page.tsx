    ...
    <main className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="section-card rounded-[28px] p-6">
              ...
              <div className="mt-5 space-y-3">
                {currentShare.materials.map((material) => (
                  <div key={material.id} className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-700 transition hover:border-[var(--active-border)]">
                    <div>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button onClick={() => void handlePreviewMaterial(material)} className="rounded-full bg-[var(--accent-soft)] px-3 py-1 text-xs font-semibold text-[var(--accent-contrast)]">打开资料</button>
                      <button onClick={() => void handleDownloadMaterial(material)} className="ui-pill rounded-full px-3 py-1 text-xs font-semibold">下载</button>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="glass-panel rounded-[32px] px-6 py-8 md:px-8">
        ...
        <div className="mt-4 flex items-center justify-between gap-3">
          <p className={`text-sm ${message.includes("失败") ? "text-rose-700" : "text-slate-500"}`}>{message || "可以请求课堂 PPT、讲义、板书材料、带批注版本或无批注版本。"}</p>
