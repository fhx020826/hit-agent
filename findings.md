...
### 2026-04-14 首页文案、语言切换与顶部条
- 首页最初的问题不只在配色，还包括：
  - 模块说明过长，用户需要自己从段落里找功能重点
  - 首页主按钮文字没有真正居中
  - 语言切换后首页与登录弹窗仍残留中文
  - 顶部上下文条使用 `sticky`，滚动时会挡住下面内容
- 本轮修复方式：
  - 首页所有主入口文案压缩成短说明，只保留模块功能
  - 首页接入语言上下文，主海报、卡片、按钮和说明区都改为随语言切换
  - 登录弹窗接入语言上下文，避免英文界面里弹出中文弹窗
  - `shell-topbar` 从 `sticky` 改为 `relative`
  - `ui-pill` 与首页主按钮统一改为居中布局
- 新增防回归手段：
  - `frontend/tests/atomic-features.spec.ts` 新增 `public homepage switches language fully and top bar no longer sticks`
  - 该用例会验证：
    - 英文首页关键文案可见
    - 英文登录弹窗可见
    - `.shell-topbar` 计算样式不再是 `sticky`
    - 首页主按钮布局为居中对齐
