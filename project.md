...
- 首页文案已进一步压缩为模块功能说明，不再保留过长说明性段落；语言切换已覆盖首页和登录弹窗；顶部上下文条也已取消粘顶，避免遮挡下方信息
...
- 浏览器原子/扩展回归：
  ...
  - 结果：`13 passed`
- 最新验证日志目录：
  - `/tmp/hit-agent-verify/20260414-192210`
- 本轮说明：
  ...
  - 新增首页英文切换、按钮居中与顶部条非粘顶回归：
    - `frontend/tests/atomic-features.spec.ts` :: `public homepage switches language fully and top bar no longer sticks`
...
- 阿里云 ECS `8.152.202.171` 当前再次出现“ICMP 正常、TCP 可建连、但 SSH / HTTP 服务无响应”的状态，说明远端仍存在整机资源饱和或核心服务卡死的真实运行风险，不适合在未恢复前继续作为当前主工作面

### 当前 ECS 状态（2026-04-14 刷新）
- 用户重启实例后，ECS 已恢复 SSH 与公网访问。
- 当前服务器代码已更新到：
...
- 但这只说明“当前可运行”，不代表 `2C2G` 已达到长期稳定部署标准。
...
#### `assignment-review` 下一步执行顺序（2026-04-14 规划刷新）
- 先复用现有 `task_jobs` 体系，而不是新起一套专用异步框架
- 后端优先补齐：
  - 新任务类型常量
  - handler
  - `task-jobs` 提交接口
...
- 前端随后改为：
  - 提交任务
  - 轮询状态
  - 渲染 queued / running / failed / succeeded
- 测试按 TDD 扩展：
  - 先补后端任务测试
  - 再按真实页面语义调整 Playwright
- 完成后必须重新跑：
  - `pytest -q`
  - 浏览器回归
